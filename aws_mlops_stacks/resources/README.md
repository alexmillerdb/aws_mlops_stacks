...
The databricks CLI bundles top level is defined by file `aws_mlops_stacks/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`aws_mlops_stacks/resources/model-workflow-resource.yml`)
 - batch inference workflow (`aws_mlops_stacks/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`aws_mlops_stacks/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`aws_mlops_stacks/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`aws_mlops_stacks/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `aws_mlops_stacks/databricks.yml` and `aws_mlops_stacks/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-aws_mlops_stacks-model     | /dev-aws_mlops_stacks-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-aws_mlops_stacks-model | /staging-aws_mlops_stacks-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-aws_mlops_stacks-model    | /prod-aws_mlops_stacks-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-aws_mlops_stacks-model    | /test-aws_mlops_stacks-experiment    |
...
2. Complete the `TODO` in `aws_mlops_stacks/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `aws_mlops_stacks/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `aws_mlops_stacks/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-aws_mlops_stacks-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-aws_mlops_stacks-batch-inference-job`
that runs the notebook under `aws_mlops_stacks/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-aws_mlops_stacks-batch-inference-job`.
...
`test-aws_mlops_stacks-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-aws_mlops_stacks-batch-inference-job`.
